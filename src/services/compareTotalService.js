...
// 비교 현황 list 
export async function getCorpTotal({ offset = 0, limit = 20, order = 'my_desc' } = {}) {
  // 기업 조회 내가 원하는 부분만
  ...
  const cmp = 
    order === 'myCompareLowest'   ? ((a,b)=> (b.my_compare_total - a.my_compare_total) || (b.created_at - a.created_at)) :
    order === 'myCompareHighest'  ? ((a,b)=> (a.my_compare_total - b.my_compare_total) || (b.created_at - a.created_at)):
    order === 'compareLowest'     ? ((a,b)=> (b.compare_corp) - a.compare_corp || (b.created_at - a.created_at)):
    order === 'compareHighest'    ? ((a,b)=> (b.compare_corp) - a.compare_corp || (b.created_at - a.created_at)):
  ...
  const page = merged.slice(off, off+ lim).map(({created_at, ...rest}) => rest);
